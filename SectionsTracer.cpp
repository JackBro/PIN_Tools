...
#define TOOL_NAME "SectionsTracer"
...
/*
VOID GetProcAddressArg(CHAR * name,  handle, char * filename)
{
...
VOID BeforeFunction2Args(CHAR * name, UINT32 handle, CHAR * filename)
{
	if (filename == NULL || handle == 0) return;
	//>
	PIN_LockClient();
	fprintf(m_FunctionsFile, "%s\n", filename);
	fflush(m_FunctionsFile);
	PIN_UnlockClient();
	//<
}

VOID AfterFunction(ADDRINT ret)
{
	//>
	PIN_LockClient();
	fprintf(m_FunctionsFile, "%#x : ", ret);
	
	//PIN_LockClient();
	IMG pImg = IMG_FindByAddress(ret);
	RTN rtn = RTN_FindByAddress(ret);
	//PIN_UnlockClient();

	if (IMG_Valid(pImg) && RTN_Valid(rtn)) {
		const string func =  RTN_Name(rtn);
		const string image = IMG_Name(pImg);
		fprintf(m_FunctionsFile, "%s,%s", image.c_str(), func.c_str());
	}
	fprintf(m_FunctionsFile, "\n");
	fflush(m_FunctionsFile);
	PIN_UnlockClient();
	//<
}

VOID MonitorFunction2Args(IMG Image, const char* funcName)
{
	RTN cfwRtn = RTN_FindByName(Image, funcName);
	if (RTN_Valid(cfwRtn))
	{
		RTN_Open(cfwRtn);
		RTN_InsertCall(cfwRtn, IPOINT_BEFORE, (AFUNPTR)BeforeFunction2Args,
		...
		RTN_InsertCall(cfwRtn, IPOINT_AFTER, (AFUNPTR)AfterFunction,
...
VOID ImageLoad(IMG Image, VOID *v)
{
	...
	MonitorFunction2Args(Image, "GetProcAddress");
}
...
int main(int argc, char *argv[])
{
	...
	string funcFileName = KnobOutputFile.Value() + ".funct";
	if (fileName.empty()) fileName = "output.txt";

	m_BlocksFile = fopen(fileName.c_str(), "w");
	m_FunctionsFile = fopen(funcFileName.c_str(), "w");
